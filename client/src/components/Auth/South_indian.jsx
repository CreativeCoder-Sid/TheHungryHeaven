...
const South_indian = () => {
  ...
  useEffect(() => {
    const fetchSouthFoods = async () => {
      try {
        const res = await API.get('/foods/category/South Indian');
        setFoods(res.data);
      } catch (error) {
        console.error('Failed to fetch South Indian food items:', error);
      }
    };
  ...
  return (
    <div className="South">
      <h1 id='bengali-food-heading' data-aos="zoom-in">South Indian Foods</h1>
